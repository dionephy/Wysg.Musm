...
public sealed class BooHillRepository
{
    ...
    private static async Task<string> EnsureWritableDatabaseAsync()
    {
        ...
        {
            throw new FileNotFoundException("Legacy database not found in packaged output.", sourcePath);
        }
    ...
    public async Task<IReadOnlyList<HouseView>> GetHousesAsync(FilterOptions filters)
    {
        ...
        if (!string.IsNullOrWhiteSpace(filters.BuildingNumber))
        {
            whereClauses.Add("h.building_number LIKE $building");
            command.Parameters.AddWithValue("$building", $"%{filters.BuildingNumber}%");
        }

        if (!string.IsNullOrWhiteSpace(filters.UnitNumber))
        {
            whereClauses.Add("h.unit_number LIKE $unit");
            command.Parameters.AddWithValue("$unit", $"%{filters.UnitNumber}%");
        }

        if (!string.IsNullOrWhiteSpace(filters.Area))
        {
            whereClauses.Add("h.area = $area");
            command.Parameters.AddWithValue("$area", filters.Area);
        }
        ...
        var orderBy = filters.SortField switch
        {
            SortField.Building => "building_number COLLATE NOCASE",
            SortField.PriceRange => "min_price",
            _ => "h.house_id"
        };

        var orderDir = filters.SortDirection == SortDirection.Descending ? "DESC" : "ASC";
        var orderClause = orderBy == "min_price"
            ? "(min_price IS NULL) ASC, min_price " + orderDir + ", max_price " + orderDir
            : orderBy + " " + orderDir;
  ...
  FROM house h";
    ...
    public async Task EnsureAreaPatchedAsync()
    {
        ...
        command.CommandText = "UPDATE house SET area = $area WHERE area IS NOT $area";
    ...
    public async Task<long> UpsertHouseAsync(HouseEdit house)
    {
        ...
        if (house.HouseId == 0)
        {
            command.CommandText = @"INSERT INTO house (cluster_id, building_number, unit_number, area, is_sold, is_favorite, value, value_estm, rank, rank_estm)
VALUES ($cluster_id, $building_number, $unit_number, $area, $is_sold, $is_favorite, $value, $value_estm, $rank, $rank_estm);
