...
HWND: 0x67890 (example) ¡Á 0x12345 (bookmark)
```
...
GetForegroundWindow() ¡æ 0x12345 (PACS main window)
Cached bookmark HWND  ¡æ 0x12345 (PACS main window)
...
GetForegroundWindow()     ¡æ 0x12345 (PACS main window)
GetFocusedWindowHandle()  ¡æ 0x67890 (RichEdit20W textbox)
Cached bookmark HWND      ¡æ 0x12345 (PACS main window)
...
```
GetFocusedWindowHandle() ¡æ 0x12345 (main window)
Cached bookmark HWND     ¡æ 0x12345 (main window)
Result: ? Autofocus triggers correctly
```

**Test 2: Click exactly on bookmarked patient list**
```
GetFocusedWindowHandle() ¡æ 0x12345 (patient list)
Cached bookmark HWND     ¡æ 0x12345 (patient list)
Result: ? Autofocus triggers correctly
```
...
```
GetFocusedWindowHandle() ¡æ 0xABCDE (search textbox)
Cached bookmark HWND     ¡æ 0x12345 (main window)
Result: ? Autofocus does NOT trigger (correct)
```
...
```
GetFocusedWindowHandle() ¡æ 0xDEF01 (filter combobox)
Cached bookmark HWND     ¡æ 0x12345 (main window)
Result: ? Autofocus does NOT trigger (correct)
```
...
```
GetFocusedWindowHandle() ¡æ 0x23456 (OK button)
Cached bookmark HWND     ¡æ 0x12345 (main window)
Result: ? Autofocus does NOT trigger (correct)
```
...
GetForegroundWindow() ¡æ Dialog HWND (different from bookmark)
GetFocusedWindowHandle() ¡æ Dialog control HWND
...
- **Key Consumption**: See `CRITICAL_FIX_2025-02-11_KeyConsumptionAndDelivery.md`
- **Bookmark Caching**: See `CRITICAL_FIX_2025-02-11_EditorAutofocusBookmarkCaching.md`
- **Feature Overview**: See `SUMMARY_2025-02-11_EditorAutofocusComplete.md`
...
- [x] Bookmark parent window ¡æ child textbox does NOT trigger
- [x] Bookmark parent window ¡æ child button does NOT trigger
- [x] Bookmark parent window ¡æ child dialog does NOT trigger
- [x] Bookmark parent window ¡æ parent window DOES trigger
- [x] Bookmark specific control ¡æ that control DOES trigger
- [x] Bookmark specific control ¡æ sibling control does NOT trigger
