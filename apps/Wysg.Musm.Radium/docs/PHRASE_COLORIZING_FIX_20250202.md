...
弛           ⊿                                                     弛
弛  AzureSqlSnomedMapService  ∠ Direct SQL access (deprecated)    弛
弛           ⊿                                                     弛
弛  Azure SQL Database                                             弛
弛                                                                 弛
弛  ? Problem: App.xaml.cs registered "UseApiClients = true"     弛
弛     but ISnomedMapService still used direct SQL                弛
弛     ⊥ API calls for phrases/snippets/hotkeys BUT direct SQL     弛
弛        for SNOMED mappings ⊥ inconsistent!                      弛
...
弛  _snomedMapService.GetMappingsBatchAsync(phraseIds)             弛
弛           ⊿                                                     弛
弛  ApiSnomedMapServiceAdapter  ∠ NEW ADAPTER (API-based)         弛
弛           ⊿                                                     弛
弛  SnomedApiClient (HttpClient wrapper)                           弛
弛           ⊿                                                     弛
弛  GET /api/snomed/mappings?phraseIds=1&phraseIds=2&...           弛
弛           ⊿                                                     弛
弛  Radium.Api ⊥ SnomedController                                  弛
弛           ⊿                                                     弛
弛  SnomedRepository ⊥ Azure SQL                                   弛
弛                                                                 弛
弛  ? Solution: Consistent API access for ALL features            弛
弛     ⊥ Phrases, Snippets, Hotkeys, SNOMED all use API           弛
戌式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式式戎
```
