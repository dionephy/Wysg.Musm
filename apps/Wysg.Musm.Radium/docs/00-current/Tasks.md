...
- [X] T515 Fix snippet completion display to show "{trigger} ¡æ {description}" instead of "{trigger} ¡æ {snippet text}" in MusmCompletionData and EditorCompletionData (FR-362).
...
- [X] T493 Add "Get HTML" button to AutomationWindow ¡æ Crawl Editor toolbar and wire Click to `OnGetHtml` (FR-339).
...
- [X] T497 Implement smart HTML decoding (`HttpGetHtmlSmartAsync`) with charset detection (header ¡æ meta) and CP949 fallback when UTF-8 looks corrupted; use it for both button and procedure `GetHTML` (FR-340).
...
- [X] T486 Add "Spy" button next to "Save Automation" in Settings ¡æ Automation tab (XAML) (FR-335).
...
- [X] T488 Update Spec/Plan/Tasks with FR-335 and change log entry for Settings ¡æ Automation Spy button.
...
- [X] T725 Add `AddPreviousStudy` to Settings ¡æ Automation available modules.
...
- [X] T761 Add `tenant_id` column (FK ¡æ app.tenant) to `med.patient`; change unique to `(tenant_id, patient_number)`.
- [X] T762 Add `tenant_id` column (FK ¡æ app.tenant) to `med.rad_studyname`; change unique to `(tenant_id, studyname)`.
...
- [X] T783 Settings ¡æ PACS tab: remove unsupported Actions column (Rename/Remove buttons).
- [X] T784 Settings ¡æ PACS tab: remove Close button below grid; rely on window close.
...
- [ ] V241 New Study pane set to only `ShowTestMessage` ¡æ pressing New shows the message and does not lock study.
- [ ] V242 Add `LockStudy` to New Study pane explicitly ¡æ pressing New locks study as designed.
...
- [ ] V253 Load phrase snapshot into EditorControl ¡æ verify phrases highlight with correct colors.
- [ ] V254 Type text with snapshot phrases ¡æ verify #4A4A4A highlighting.
- [ ] V255 Type text with non-snapshot phrases ¡æ verify red highlighting.
- [ ] V256 Type multi-word phrases (2-5 words) ¡æ verify entire phrase highlights as single unit.
- [ ] V257 Update phrase snapshot at runtime ¡æ verify highlighting updates immediately.
- [ ] V258 Scroll document ¡æ verify highlighting only processes visible text regions.
...
- [ ] T921 Add SNOMED search panel to Settings ¡æ Global Phrases tab with search textbox, results grid, and map button (FR-909, future work).
...
- [ ] T924 Add SNOMED search and mapping UI to Settings ¡æ Phrases tab (account-specific) (FR-910, future work).
...
- [ ] V301 Call snomed.upsert_concept with test data ¡æ verify concept cached with correct fields.
- [ ] V302 Call radium.map_global_phrase_to_snomed with global phrase ¡æ verify mapping created.
- [ ] V303 Call radium.map_phrase_to_snomed with account phrase ¡æ verify mapping created.
- [ ] V304 Attempt to map account phrase via global procedure ¡æ verify RAISERROR.
- [ ] V305 Attempt to map global phrase via account procedure ¡æ verify RAISERROR.
- [ ] V306 Delete phrase ¡æ verify mapping CASCADE deleted.
- [ ] V307 Attempt to delete concept with existing mappings ¡æ verify FK RESTRICT violation.
- [ ] V308 Update mapping fields ¡æ verify updated_at changes; update non-tracked fields ¡æ verify updated_at unchanged.
- [ ] V309 Query v_phrase_snomed_combined ¡æ verify UNION ALL returns global and account mappings with correct mapping_source.
...
- [X] V311 Open "Link SNOMED" from Global Phrases ¡æ verify search box pre-filled with phrase text.
- [X] V312 Press Enter or click Search ¡æ verify Snowstorm search executes with pre-filled text.
- [X] V313 Map button is disabled when no concept selected.
- [X] V314 Select a concept from search results ¡æ verify Map button enables immediately.
- [X] V315 Click Map button ¡æ verify mapping saves successfully and confirmation appears.
- [X] V316 Select different concept ¡æ verify Map button remains enabled.
- [X] V317 Clear selection ¡æ verify Map button disables.
...
- [X] T943 Validate process name not empty, chain not empty, first node has ¡Ã1 enabled attribute (FR-922).
...
- [ ] V320 Open PACS with main + toolbar windows; capture bookmark with ClassName enabled ¡æ verify root matches main window consistently across 5 reopens.
- [ ] V321 Edit bookmark to leave only 1 attribute enabled on first node ¡æ verify validation error prevents save.
- [ ] V322 Enable second attribute ¡æ verify validation passes and save succeeds.
- [ ] V323 Capture bookmark with unique AutomationId ¡æ verify similarity scoring selects correct root when multiple candidates exist.
- [ ] V324 Simulate ControlTypeId change ¡æ verify relaxed match fallback succeeds with trace message.
- [ ] V325 Resolve bookmark with trace ¡æ verify trace shows attribute match results (Name=true, Class=true, Auto=false, Ct=true) and timing info (e.g., "Step 0: Accept root... (12 ms)").
- [ ] V326 Resolve bookmark after ClassName filter ¡æ verify trace shows "ClassName filter applied: N roots remain".
- [ ] V327 Verify bookmarks saved before fix continue to work (no regression for existing bookmarks).
- [ ] V328 Resolve bookmark with multiple steps ¡æ verify each step shows timing (e.g., "Step 2: Completed (45 ms)").
- [ ] V329 Click Validate button in AutomationWindow ¡æ verify diagnostic table includes timing column showing per-step milliseconds.
- [ ] V330 Click "Resolve" with trace on slow bookmark ¡æ verify trace shows retry breakdown with query time, retry delay, and attempt count for each step.
- [ ] V331 Click "Validate" on any bookmark ¡æ verify status textbox shows last 100 lines of trace with detailed timing for all steps (even on success).
- [ ] V332 Click "Validate" on Calculator bookmark ¡æ verify trace shows "Detected 'not supported' error, skipping remaining retries" and resolution completes in <1 second.
- [ ] V333 Compare Calculator bookmark timing before/after fix ¡æ verify resolution is 4-6x faster (from ~2900ms to ~500-800ms).
...
- [X] V271 Related study missing name/datetime ¡æ `AddPreviousStudy` aborts gracefully; status shows reason.
- [X] V272 Related study equals current by name+datetime ¡æ `AddPreviousStudy` aborts; status shows reason.
...
- [ ] V272 Open AutomationWindow Map-to dropdown ¡æ verify "Screen_main current study tab" and "Screen_sub previous study tab" listed (pending manual XAML update)
- [ ] V273 Map Screen_MainCurrentStudyTab bookmark to PACS main screen current study area ¡æ verify saved and resolves correctly (pending manual XAML update)
- [ ] V274 Map Screen_SubPreviousStudyTab bookmark to PACS sub screen previous study area ¡æ verify saved and resolves correctly (pending manual XAML update)
- [ ] V275 Open AutomationWindow Custom Procedures ¡æ verify "Set current study in main screen" and "Set previous study in sub screen" listed (pending manual XAML update)
- [ ] V276 Select SetCurrentStudyInMainScreen method ¡æ verify auto-seeded with ClickElement operation targeting Screen_MainCurrentStudyTab (pending manual XAML update)
- [ ] V277 Run SetCurrentStudyInMainScreen procedure ¡æ verify click occurs at bookmark center and preview shows coordinates (pending manual XAML update)
- [ ] V278 Open AutomationWindow Operations dropdown ¡æ verify ClickElement operation listed (pending manual XAML update)
- [ ] V279 Select ClickElement operation ¡æ verify Arg1 preset to Element type, Arg2/Arg3 disabled, Run button clickable after mapping bookmark (pending manual XAML update)
...
- [X] T972 Rewrite ResolveElement() with retry loop:
  - Check cache ¡æ validate with IsElementAlive() ¡æ return if valid
  - Clear stale cache entries immediately
  - Resolve fresh from bookmark ¡æ validate before caching
...
- [ ] V291 Stale cache: PACS window hierarchy changes ¡æ cached element becomes stale ¡æ automatic retry resolves fresh element
- [ ] V292 Transient failure: UI busy during resolution ¡æ first attempt fails ¡æ retry after 150ms succeeds
- [ ] V293 Permanent failure: bookmark points to non-existent element ¡æ all 3 attempts fail ¡æ operation reports "(no element)" error
- [ ] V294 Performance: measure resolution time with cache hit (<10ms) vs. cache miss with retry (<1 second for 3 attempts)
- [ ] V295 Integration: run automation sequence with 10+ operations ¡æ verify no stale element errors
...
- [ ] V300 Exact match success: bookmark resolves on first attempt ¡æ no relaxation, no retry delay
- [ ] V301 ControlType relaxation: PACS UI update changes control types ¡æ second attempt succeeds
- [ ] V302 ClassName relaxation: major UI rearrangement ¡æ third attempt succeeds with Name + AutomationId only
- [ ] V303 Complete failure: bookmark completely invalid ¡æ all 3 attempts fail, return (IntPtr.Zero, null)
- [ ] V304 Performance: measure first attempt (~100ms), retry overhead (150-300ms only when needed)
- [ ] V305 Integration: run automation sequence where one bookmark requires relaxation ¡æ verify automatic recovery
...
- [ ] T996 Document legacy pattern: AutomationId ¡æ ClassName fallback
...
### FR-964: Dual Pattern Fallback (Invoke ¡æ Toggle)
- [X] T1001 Verify Invoke ¡æ Toggle fallback already implemented in ProcedureExecutor
...
- [ ] V310 Normal case: pure index node with UseIndex=true, all attributes=false ¡æ resolves to correct child
- [ ] V311 Out of range: index=5 but only 3 children ¡æ fails gracefully with clear error
- [ ] V312 Descendants scope: pure index with Scope=Descendants ¡æ skipped (not supported)
- [ ] V313 Legacy replication: test with ePanWorklistToolBar pattern (index 1 from worklist) ¡æ matches legacy behavior
- [ ] V314 Integration: full bookmark chain with pure index step ¡æ resolves successfully
- [ ] V315 Performance: pure index vs. attribute matching ¡æ pure index faster (<10ms)
...
- [ ] V420 Open "Manage Studyname Techniques" window ¡æ verify "Save as New Combination" button is disabled initially
- [ ] V421 Select Prefix, Tech, Suffix ¡æ click "Add to Combination" ¡æ verify button becomes enabled immediately
- [ ] V422 Add 2-3 more techniques ¡æ verify button remains enabled
- [ ] V423 Click "Save as New Combination" ¡æ verify saves successfully
...
- [ ] V427 Repeat add/save cycle 2-3 times ¡æ verify button enables/disables correctly each time
- [ ] V428 Add items then close window without saving ¡æ verify items not persisted
- [ ] V429 Test with studyname not set ¡æ verify button remains disabled even when adding items (safeguard)
- [ ] V430 Performance test: add 10 items rapidly ¡æ verify button state updates responsively without lag
...
- [ ] V480 Open Main Window in portrait mode ¡æ verify Chief Complaint upper border aligns with Chief Complaint (PR)
...
- [ ] V484 Type multi-line content into Chief Complaint ¡æ verify both main and proofread textboxes grow proportionally
- [ ] V485 Type multi-line content into Patient History ¡æ verify alignment maintained during height changes
- [ ] V486 Rotate to landscape mode ¡æ verify same alignment behavior in gridSideTop panel
- [ ] V487 Scroll proofread column ¡æ verify main column scrolls in sync
- [ ] V488 Scroll main column ¡æ verify operates independently (no feedback sync back to proofread)
- [ ] V489 Toggle Reverse Reports ¡æ verify columns swap and alignment maintained
- [ ] V490 Toggle Reverse back ¡æ verify alignment restored
- [ ] V491 Add 50+ lines to both columns ¡æ verify scroll synchronization works smoothly without lag
- [ ] V492 Verify no visual glitches or overlap when textboxes have different content lengths
- [ ] V493 Resize window horizontally ¡æ verify textboxes and proofread columns resize proportionally
- [ ] V494 Test with empty fields ¡æ verify alignment maintained with minimal heights
- [ ] V495 Compare portrait and landscape layouts ¡æ verify consistent behavior across orientations
...
- [ ] V440 Open "Manage Studyname Techniques" window ¡æ verify both "Current Combination" and "All Combinations" ListBoxes are visible
...
- [ ] V445 Double-click an item in Current Combination ¡æ verify it removes immediately without confirmation
- [ ] V446 Verify Save button disables when last item removed via double-click
- [ ] V447 Double-click multiple items rapidly ¡æ verify all remove correctly
...
- [ ] V452 Double-click a combination in All Combinations with empty Current Combination ¡æ verify techniques load
...
- [ ] V455 Add 2 techniques manually, then double-click a combination ¡æ verify new techniques append (sequence continues)
- [ ] V456 Double-click same combination twice ¡æ verify no duplicates added second time
- [ ] V457 Manually add "axial T1", then double-click combination containing "axial T1" ¡æ verify that technique skipped
- [ ] V458 Double-click combination with techniques having null prefix/suffix ¡æ verify techniques load correctly
- [ ] V459 Verify Save button enables after loading techniques from All Combinations
- [ ] V460 Load techniques from All Combinations, modify list (add/remove), then save ¡æ verify new combination created
