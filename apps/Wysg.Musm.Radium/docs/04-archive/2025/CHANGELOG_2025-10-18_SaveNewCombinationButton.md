...
### Why This Happens
1. User adds technique �� `CurrentCombinationItems.Count` changes from 0 to 1
...
1. ? **Initial State**: Open window �� button disabled (no items)
2. ? **Add First Item**: Add technique �� button enables immediately
3. ? **Add More Items**: Add 2-3 more �� button remains enabled
4. ? **Save**: Click save �� combination saved successfully
5. ? **After Save**: List clears and button disables
6. ? **Repeat Cycle**: Add/save 2-3 times �� button enables/disables correctly each cycle
