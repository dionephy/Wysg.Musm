...
[Automation][GetReportedReport] Radiologist: '�赿��'
...
### Reading JSON
- ? Old JSON with `findings`/`conclusion` �� Reads correctly (fallback logic)
- ? New JSON with `header_and_findings`/`final_conclusion` �� Reads correctly (preferred)
- ? JSON with both sets of keys �� Reads standardized names first
