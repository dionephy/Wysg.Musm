...
**Date**: 2025-11-02  
**Issue**: Hyphens being removed from medical terminology (A2-A3 �� A22A3)  
...
private string ApplyReportifyBlock(string input, bool isConclusion)
{
    ...
    input = input.Replace('\u2013', '-'); // En-dash (?) �� Hyphen (-)
    input = input.Replace('\u2014', '-'); // Em-dash (?) �� Hyphen (-)
    input = input.Replace('\u2212', '-'); // Minus sign (?) �� Hyphen (-)
...
Output: "A2-A3 and B1-B2 segments" (both �� hyphens)
? PASS
```
...
1. **Smart Quotes**: Could add similar normalization for `"` �� `"` and `'` �� `'`
2. **Non-Breaking Spaces**: Could normalize `\u00A0` �� regular space
...
input = input.Replace('\u201C', '"'); // Left double quote �� ASCII quote
input = input.Replace('\u201D', '"'); // Right double quote �� ASCII quote
input = input.Replace('\u2018', '\''); // Left single quote �� ASCII apostrophe
input = input.Replace('\u2019', '\''); // Right single quote �� ASCII apostrophe
input = input.Replace('\u00A0', ' '); // Non-breaking space �� regular space
```
...
1. **apps/Wysg.Musm.Radium/ViewModels/MainViewModel.ReportifyHelpers.cs**
   ...
   - Converts U+2013, U+2014, U+2212 �� U+002D (ASCII hyphen)
