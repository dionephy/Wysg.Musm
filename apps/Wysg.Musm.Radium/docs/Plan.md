...
## Change Log Addition (2025-10-09 - Hotkey Display Polish)
- Adjusted hotkey completion display to include spaces around arrow (" → ") and to truncate multi-line expansions to first line with ellipsis (FR-297, FR-298).
- Suppressed completion item tooltips by emitting null/empty Description from completion data (FR-299).

## Change Log Addition (2025-10-09 - Editor Hotkey Completion Integration)
- EditorControl now receives a composite ISnippetProvider that merges phrase tokens with active hotkeys from IHotkeyService (FR-292, FR-295).
- Hotkeys render in completion list as "{trigger}→{expansion}" using MusmCompletionData.Hotkey with Replacement bound to expansion (FR-296).
- On selection, the completion engine replaces the current word range with the expansion and advances caret (FR-293).
- MainViewModel.InitializeEditor now preloads hotkey snapshot and injects the composite provider; phrases cache preloaded as before (FR-294).

## Change Log Addition (2025-10-09 - Combined Phrases in Completion)
- Initialize editor by preloading combined (global + account) phrases into cache so completion shows global items immediately (FR-301).
- Composite provider now uses combined phrases and performs a background prefetch when cache is missing or empty (FR-300).

## Change Log Addition (2025-10-09 - Completion Navigation Fix)
- Fixed first Down/Up key press to move selection to the next/previous item immediately if a selection already exists; removed prior guard that required two presses (FR-302). Home/End logic unchanged.
- Overrode ListBox Home/End handling in completion window; keys now move caret in editor and do not change ListBox selection. Popup closes if caret leaves replace range (FR-303).

## Change Log Addition (2025-10-09 - Home/End Interception)
- Added window-level PreviewKeyDown handler to intercept Home/End and redirect to editor caret; prevents ListBox handling entirely (FR-304).

## Change Log Addition (2025-10-09 - First Down Wrap Fix)
- Handled Up/Down inside completion ListBox PreviewKeyDown to select next/previous predictably; prevents initial Down from wrapping to last item when no prior selection (FR-305).

## Change Log Addition (2025-10-09 - Predictable Initial Selection)
- Disabled exact-match auto-selection in completion popup; default-selects the first item so the first Down key always moves to the second item (FR-306).
