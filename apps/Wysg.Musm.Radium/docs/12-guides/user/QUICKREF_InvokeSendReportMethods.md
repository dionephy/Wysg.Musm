...
## Where to Find Them?
**UI Spy Window** → **Custom Procedures** section → **PACS Method** dropdown → Look for new items at the bottom of the list

## Quick Configuration Steps

### Step 1: Open UI Spy
- From Radium main window: Tools menu → UI Spy
...
3. Common steps:
   ```
   Operation: IsVisible     Arg1: ReportTextEditor (Element)    → Check if report is open
   Operation: ClickElement  Arg1: SendReportButton (Element)    → Click send button
   Operation: Delay         Arg1: 500 (Number)                   → Wait for dialog
   Operation: ClickElement  Arg1: ConfirmButton (Element)       → Confirm send
   ```
...
3. Common steps:
   ```
   Operation: IsVisible     Arg1: ErrorDialog (Element)          → Check for error
   Operation: ClickElement  Arg1: CloseErrorButton (Element)    → Close error dialog
   Operation: Delay         Arg1: 1000 (Number)                  → Wait longer
   Operation: ClickElement  Arg1: SendReportButton (Element)    → Retry send
   Operation: Delay         Arg1: 500 (Number)                   → Wait for response
   ```
4. Click "Save"
5. Click "Run" to test
