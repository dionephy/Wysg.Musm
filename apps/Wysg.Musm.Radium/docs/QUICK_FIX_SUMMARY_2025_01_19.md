...
2. Run automation ?? toggle should turn ON
3. Click toggle manually ?? should turn OFF
4. Run automation again ?? toggle should turn ON
...
### Root Cause
The `SearchResultsList` bookmark in AutomationWindow is **mapped to the wrong UI element**.

### How to Fix (5 minutes)
1. Open **AutomationWindow** (click Spy button in MainWindow status bar)
...
5. Click **Run** in AutomationWindow to test ?? mouse should now move to list
6. Test automation sequence ?? should work correctly
...
| ResultsListSetFocus | Bookmark Mapping | ?? USER ACTION | Re-map in AutomationWindow |

## Next Steps
1. ? Rebuild solution (Issue 1 fix applied)
2. ?? Open AutomationWindow and re-map `SearchResultsList` bookmark (5 min)
3. ? Test both fixes
