...
**Symptom:**
- Snippets visible in Settings �� Snippets tab
...
**Impact:**
- Every keystroke triggered completion window �� API call
...
- Hotkeys also not cached �� double API penalty
...
- Return from cache on `GetActiveSnippetsAsync()` �� no API call per keystroke
...
public void InitializeEditor(EditorControl editor)
{
    ...
    _ = Task.Run(async () =>
    {
        ...
        try
        {
            ...
            // Preload hotkeys snapshot �� fills cache
            await _hotkeys.PreloadAsync(accountId);
            
            // Preload snippets snapshot �� fills cache
...
1. **Snippet Upsert** �� Cache updated synchronously
2. **Snippet Toggle** �� Cache updated synchronously
3. **Snippet Delete** �� Cache entry removed
4. **Hotkey Upsert** �� Cache updated synchronously
5. **Hotkey Toggle** �� Cache updated synchronously
6. **Hotkey Delete** �� Cache entry removed
...
- Settings window saves �� `RefreshSnippetsAsync()` / `RefreshHotkeysAsync()`
- Account switch �� Cache cleared, preload new account
...
**Snippets:**
- [x] ? Add snippet in Settings �� Snippets
- [x] ? Type trigger in EditorFindings �� completion shows snippet
- [x] ? Select snippet �� expands correctly with placeholders
- [x] ? Tab through placeholders �� mode 1/2/3 all working
- [x] ? Toggle snippet inactive in Settings �� completion hides it
- [x] ? Delete snippet in Settings �� completion removes it
- [x] ? No lag or delay during typing

**Hotkeys:**
- [x] ? Add hotkey in Settings �� Hotkeys
- [x] ? Type trigger in EditorFindings �� completion shows hotkey
- [x] ? Select hotkey �� replaces with expansion text
- [x] ? Toggle hotkey inactive �� completion hides it
- [x] ? Delete hotkey �� completion removes it
- [x] ? No lag or delay during typing

**Performance:**
- [x] ? Type rapidly (60+ WPM) �� no sluggishness
...
_ = Task.Run(async () =>
{
    ...
    {
        // Silent failure �� completion shows phrases only (from PhraseCache)
    }
});
```
...
### Results
? **Performance** - 40-80x faster completion (API �� <1ms cache lookup)  
