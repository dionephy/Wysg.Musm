...
{
    /// <summary>
    /// OperationExecutor partial class: Encoding detection and Korean mojibake repair.
    /// Contains sophisticated encoding logic for handling Korean, UTF-8, CP949, and mixed encodings.
    /// </summary>
    internal static partial class OperationExecutor
    {
        ...
        private static string? TryRepairLatin1ToUtf8(string text)
        {
            ...
            try { return Encoding.UTF8.GetString(buf); }
            catch { return null; }
        }
        ...
        private static string? TryRepairLatin1ToCp949(string text)
        {
            ...
            var buf = new byte[text.Length];
            for (int i = 0; i < text.Length; i++) buf[i] = (byte)(text[i] & 0xFF);
            try { return Encoding.GetEncoding("cp949").GetString(buf); }
            catch { return null; }
        }
        ...
        private static string RepairLatin1Runs(string s)
        {
            ...
            foreach (Match m in rx.Matches(s))
            {
                ...
                var chunk = m.Value;
                string best = chunk;
                int bestHan = CountHangul(chunk);
                int bestRep = CountReplacement(chunk);
                void Consider(string? c)
                {
                    if (string.IsNullOrEmpty(c)) return;
                    int han = CountHangul(c);
                    int rep = CountReplacement(c);
        ...
        private static string NormalizeKoreanMojibake(string? s)
        {
            ...
            string best = repairedRuns;
            int bestHan = CountHangul(repairedRuns);
            int bestRep = CountReplacement(repairedRuns);
            void Consider(string? c)
            {
                if (string.IsNullOrEmpty(c)) return;
                int han = CountHangul(c);
        ...
        private static string DecodeBest(byte[] bytes, List<Encoding> encs)
        {
            ...
            foreach (var enc in encs)
            {
                ...
                bool better = rep < bestRep || (rep == bestRep && han > bestHangul);
                if (better)
                {
                    bestRep = rep;
                    bestHangul = han;
